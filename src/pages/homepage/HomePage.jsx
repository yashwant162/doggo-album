...
export default function HomePage() {
  ...
  return (
    <div>
      <h2 className="text-4xl text-slate-200 mb-10 mt-10">
        Home Page {`{${breed.charAt(0).toUpperCase() + breed.slice(1)}}`}
      </h2>
      {loading ? (<div className="flex justify-center items-center">
        <div className="animate-spin rounded-full border-t-4 border-slate-200 border-opacity-75 h-20 w-20"></div>
      </div>):(<div className="gap-x-6 lg:columns-5 sm:columns-1 md:columns-3">
        {images.map((imageUrl) => (
          <div
            key={imageUrl}
