        ...
        {breedsList.map((breedName) => {
          ...
          return (
            <div
              ...
              className={`${ breed === breedName ? 'border-2 border-slate-200' : ''} bg-gray-800 m-4 p-4 rounded-md cursor-pointer text-xl text-gray-300 flex items-center gap-2 
              border-2 border-gray-800 hover:border-slate-200`}          
            >
              {displayIcon}
              <div className="">{breedName.charAt(0).toUpperCase() + breedName.slice(1)}</div>
            </div>
          );
        })}
      </div>
